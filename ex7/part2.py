...
def is_pair(counts):
    if 'J' in counts:
        jokers = counts['J']
        counts_sorted = sorted(counts.items(), key=lambda x: x[1] * 100 + strength.index(x[0]), reverse=True)
        counts_sorted = [x for x in counts_sorted if x[0] != 'J']
        highest_count = counts_sorted[0][1]
...
def is_full_house(counts):
    ...
    return is_two_pair(counts) and 'J' in counts
